...
1. **Tier-2 IPD likelihood wired into fit.** *Status: ✅ closed in v0.2.0 (commit `7a7469e`).* Per-individual reconstructed time-to-event records now contribute to the PyMC fit via an exponential AFT block sharing mu and beta with the Tier-1 Normal block. `eta_i = b0 + arm_i * (mu + X_i . beta)`; `log L_i = d_i * eta_i - exp(eta_i) * t_i`. Regression test proves posterior strictly tightens when Tier-2 added. Simplification acknowledged: exponential (constant-hazard) instead of the full discrete-time Cox-via-Poisson form; full form is v0.3 if real-data fits show constant-hazard biting.
...
3. **G-formula on natural scale.** *Status: ✅ closed in v0.1.3 (commit `57ca35d`).* `transport/g_formula.py` now averages on the natural HR scale per spec §7 (`log( sum_c P_T(c) * exp(theta_c) )`), eliminating the Jensen-bias of the v0.1 log-scale implementation. Note: `g_formula` is currently a unit-tested module not called by the pipeline — fix corrects the contract for when a future stage invokes it.
...
**v0.2.0 ship status (2026-04-24):** all four spec §3a deferrals are closed. The engine, schema, ingest, diagnostics gate, leakage gate, held-out validation runner, natural-scale g-formula, identifiable model, Tier-2 IPD exponential-AFT likelihood, single-file viewer, and end-to-end synth pipeline are all complete. Next v0.3 work is either (a) real FIDELIO/FIGARO/FINEARTS subgroup HR transcription to turn synthetic fixtures into an actual held-out validation against §9 pass criteria, or (b) upgrade Tier-2 from exponential AFT to full discrete-time Cox-via-Poisson if real-data fits show constant-hazard biting.
