...
**v0.1 ship status:** the engine, schema, ingest, diagnostics gate, leakage gate, held-out validation runner, natural-scale g-formula, single-file viewer, and end-to-end synth pipeline (with posterior wired through) are complete. Only item 1 (Tier-2 Poisson likelihood wired into the fit) remains v0.2 work.
